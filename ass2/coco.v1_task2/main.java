...
class ASTMaker extends AbstractParseTreeVisitor<AST> implements cocoVisitor<AST> {
	...
	return new DataTypeDefImpl(ctx.name.getText(),s.substring(1,s.length()-1),list);
    }
    public AST visitAlternative(cocoParser.AlternativeContext ctx){
	List<ArgumentImpl> list=new ArrayList<ArgumentImpl>();
	for(cocoParser.ArgumentContext c: ctx.as.as)
	    list.add((ArgumentImpl)visit(c));
	return new AlternativeImpl(ctx.cons.getText(),list,ctx.code.getText());
    }
	...
	return new ArgumentImpl(ctx.type.getText(),ctx.name.getText());
    }
}
